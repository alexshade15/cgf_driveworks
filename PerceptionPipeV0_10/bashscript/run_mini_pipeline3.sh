...
if [[ -n ${BUILD_WORKSPACE_DIRECTORY} ]] || [[ -n ${RUNFILES_DIR} ]]; then
    ...
    if [[ -d $DW_TOP_PATH/run_mini_pipeline.runfiles ]]; then
        ...
        echo "Ruining no build"
        #CurrentPWD was absent
        CURNTPWD=$(pwd)
        SAMPLE_TOP_PATH=${CURNTPWD}/../
        #SAMPLE_TOP_PATH=${DW_TOP_PATH}/apps/minipipeline2
    fi
fi

# Make sure CGF package is installed
if [[ $RUN_WITH_DAZEL -ne 1 ]]; then
    dpkg -V driveworks-cgf &> /dev/null 
    if [[ $? -ne 0 ]]; then
...
CMD=$(pwd)
DATA_PATH=${SAMPLE_TOP_PATH}
SMP_LOG_PATH=${SAMPLE_TOP_PATH}/logSpace/LogFolder
...
# CMD=$(pwd)
# DATA_PATH=${DW_TOP_PATH}/data/samples/minipipeline2
...
echo "LD_LIBRARY_PATH_ORIGINAL=${LD_LIBRARY_PATH}"
