...
std::unique_ptr<VisualizationNode> VisualizationNode::create(
    dw::framework::ParameterProvider& provider) {
  auto constructorArguments =
      dw::framework::createConstructorArguments<VisualizationNode>();
  VisualizationNodeParams& params = std::get<0>(constructorArguments);
  dw::framework::populateParameters<VisualizationNode>(constructorArguments,
                                                       provider);

  return dw::framework::makeUniqueFromTuple<VisualizationNode>(
      std::move(constructorArguments));
}

VisualizationNode::VisualizationNode(const VisualizationNodeParams& params,
                                     const dwContextHandle_t ctx)
    : ExceptionSafeProcessNode(
          std::make_unique<VisualizationNodeImpl>(params, ctx)) {}
