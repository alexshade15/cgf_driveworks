 ...
 * The track ball has top down view by default and is controlled by mouse and
 * key mouse: left key to rotate, wheel to zoom key: "-" to zoom out and "=" to
 * zoom in, "r" to reset to default view
 */
struct SimpleTrackBall {
 public:
  SimpleTrackBall() { Init(); }
  ...
  void UpdateEye() {
    eye[0] =
        radius * cos(vertAngle + deltaVerAngle) * cos(horAngle + deltaHorAngle);
    eye[1] =
        radius * cos(vertAngle + deltaVerAngle) * sin(horAngle + deltaHorAngle);
  ...
  const dwMatrix4f* GetModelView(const float32_t offsetX,
                                 const float32_t offsetY) {
    // Update radius, vertAngle, and horAngle (= r, theta, pi in polar
    // coordinates)
...
class VisualizationNodeImpl
    : public dw::framework::SimpleNodeT<VisualizationNode> {
 public:
  static constexpr char LOG_TAG[] = "VisualizationNode";
  using Base = SimpleNodeT<VisualizationNode>;

  // Initialization and destruction
  VisualizationNodeImpl(const VisualizationNodeParams& params,
 ...
 private:
  ...
  void preProcess3DView(const bool updateView, const bool updateProjection,
                        const bool hasGrid);
  ...
  static void onMouseUpCallback(void* instance_, int button, float x, float y,
                                int mods);
  static void onMouseDownCallback(void* instance_, int button, float x, float y,
                                  int mods);
  static void onMouseWheelCallback(void* instance_, float dx, float dy);
  static void onKeyPressCallback(void* instance_, int key, int scancode,
                                 int action, int mods);
  ...
  inline void upadteIMUString(IMUString& result, const char8_t* tag,
                              float64_t value) {
    if (std::isnan(value)) {
      return;
    }
  ...
  uint32_t m_boxNum = 0;
  const YoloClassName YOLO_CLASS_NAMES[80] = {
      "person",        "bicycle",      "car",
