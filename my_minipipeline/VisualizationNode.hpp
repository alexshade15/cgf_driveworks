...
static constexpr size_t MAX_CAMERA_NAME_LENGTH = 512;
 ...
 public:
  static constexpr auto describeInputPorts() {
    ...
    return describePortCollection(
        DW_DESCRIBE_PORT_ARRAY(dwImageHandle_t, MAX_CAMERA_COUNT, "IMAGE"_sv,
        ...
        DW_DESCRIBE_PORT_ARRAY(dwPointCloud, MAX_LIDAR_COUNT,
                               "LIDAR_POINT_CLOUD"_sv),
        DW_DESCRIBE_PORT_ARRAY(dwLidarPacketsArray, MAX_LIDAR_COUNT,
                               "LIDAR_PACKET_ARRAYS"_sv),
        DW_DESCRIBE_PORT(dwVehicleIOSafetyState, "VIO_SAFETY_STATE"_sv,
                         PortBinding::OPTIONAL),
        DW_DESCRIBE_PORT(dwVehicleIONonSafetyState, "VIO_NON_SAFETY_STATE"_sv,
                         PortBinding::OPTIONAL),  //);
#ifdef PERCEPTION_ENABLED
  ...
  static constexpr auto describeOutputPorts() {
    using namespace dw;
    using namespace dw::framework;
    return dw::framework::describePortCollection(DW_DESCRIBE_PORT(
        dwVehicleIOSafetyCommand, "VIO_SAFETY_CMD"_sv, PortBinding::OPTIONAL));
  };
  ...
  static std::unique_ptr<VisualizationNode> create(
      dw::framework::ParameterProvider& provider);

  static constexpr auto describeParameters() {
    using namespace dw;
    using namespace dw::framework;
    return describeConstructorArguments<VisualizationNodeParams,
                                        dwContextHandle_t>(
        describeConstructorArgument(
            DW_DESCRIBE_UNNAMED_ARRAY_PARAMETER_WITH_SEMANTIC(
                bool, semantic_parameter_types::CameraEnabled, MAX_CAMERA_COUNT,
                &VisualizationNodeParams::camEnable),
            DW_DESCRIBE_UNNAMED_ARRAY_PARAMETER_WITH_SEMANTIC(
                dw::core::FixedString<MAX_CAMERA_NAME_LENGTH>,
                semantic_parameter_types::CameraName, MAX_CAMERA_COUNT,
                &VisualizationNodeParams::cameraNames),
            DW_DESCRIBE_UNNAMED_ARRAY_PARAMETER_WITH_SEMANTIC(
                uint32_t, semantic_parameter_types::ImageWidth,
                MAX_CAMERA_COUNT, &VisualizationNodeParams::imageWidth),
            DW_DESCRIBE_UNNAMED_ARRAY_PARAMETER_WITH_SEMANTIC(
                uint32_t, semantic_parameter_types::ImageHeight,
                MAX_CAMERA_COUNT, &VisualizationNodeParams::imageHeight),
            DW_DESCRIBE_PARAMETER(size_t, "masterCameraIndex"_sv,
                                  &VisualizationNodeParams::masterCameraIndex),
            DW_DESCRIBE_PARAMETER(bool, "fullscreen"_sv,
                                  &VisualizationNodeParams::fullscreen),
            DW_DESCRIBE_PARAMETER(bool, "offscreen"_sv,
                                  &VisualizationNodeParams::offscreen),
            DW_DESCRIBE_PARAMETER(uint32_t, "winSizeW"_sv,
                                  &VisualizationNodeParams::winSizeW),
            DW_DESCRIBE_PARAMETER(uint32_t, "winSizeH"_sv,
                                  &VisualizationNodeParams::winSizeH),
            DW_DESCRIBE_INDEX_PARAMETER(cudaStream_t, "streamIndex"_sv,
                                        &VisualizationNodeParams::stream)),
        describeConstructorArgument(
            DW_DESCRIBE_UNNAMED_PARAMETER(dwContextHandle_t)));
  }

  VisualizationNode(const VisualizationNodeParams& params,
